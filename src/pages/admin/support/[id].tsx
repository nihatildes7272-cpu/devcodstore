...
const statuses = ["Açık", "İnceleniyor", "Çözüldü", "Kapandı"] as const;
...
export default function AdminSupportDetailPage() {
  ...
  async function loadTicket() {
    ...
    if (ticketError || !ticketData) {
      setInfo("Destek talebi bulunamadı.");
      setTicket(null);
      setMessages([]);
      setLoading(false);
      return;
  ...
  async function updateStatus(status: string) {
    ...
    if (error) {
      setInfo("Durum güncellenemedi: " + error.message);
      return;
    }
  ...
  async function sendReply(event: React.FormEvent) {
    ...
    if (!adminUser || !ticket || !reply.trim()) return;
    ...
    <main className="min-h-screen bg-[#070A12] text-white">
      <section className="mx-auto max-w-5xl px-6 py-10">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Kullanıcı: {profile?.full_name || "Kullanıcı"} — {profile?.email || "E-posta yok"}
              </p>
              <p className="mt-1 text-sm text-gray-500">
        ...
        <section className="mt-8 rounded-3xl border border-white/10 bg-white/5 p-6">
          <h2 className="text-2xl font-bold">Talep Durumu</h2>
        ...
        {ticket.status !== "Kapandı" && (
          <section className="mt-8 rounded-3xl border border-white/10 bg-white/5 p-6">
            <h2 className="text-2xl font-bold">Admin Yanıtı</h2>
